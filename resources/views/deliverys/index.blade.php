                ...
                <div class="card-body">
                    <div class="d-flex justify-content-end">
                        <div class="form-group">
                            <a href="{{ route('delivery.create') }}" class="btn btn-info btn-sm">crear registro</a>
                        </div>
                    </div>
                    <table id="example2" class="table table-bordered table-sm table-hover table-striped ">
                        ...
                        <tbody>
                            @foreach ( $deliverys as $delivery  )
                                 <tr>
                                     <td>{{$delivery->id}}</td>
                                     <td>{{$delivery->nombre_delivery}}</td>
                                     <td>{{$delivery->apellido_delivery}}</td>
                                     <td>{{$delivery->telefono_delivery}}</td>
                                     <td>{{$delivery->nro_licencia}}</td>
                                     <td>{{$delivery->placa}}</td>
                                     <td class="py-1 align-middle text-center">
                                        <div class="btn-group btn-group-sm">
                                          <a class="btn btn-warning" rel="tooltip" data-placement="top" title="Editar" href="{{ url('delivery/edit/'.$delivery->id)}}"><i class="fas fa-pencil-alt"></i></a>
                                          ...
                                          <a class="btn btn-danger"  title="Eliminar" href="{{url('delivery/destroy/'.$delivery->id)}}"><i class="fas fa-trash"></i></a>
                                        </div>
                                      </td>
                                 </tr> 
                            @endforeach
